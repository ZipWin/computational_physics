...
module roots_seeker
    ...
    implicit none
    integer, parameter :: MAX_ITER_NUM = 50
    ...
    subroutine newton_downhill(f, x_init, epsilon1, epsilon2, file_name, root, iter, stat)
        ...
        do while (.not. stop_condition)
            ...
            do while (.true.)
                ...
                if (abs(f(tmp)) < abs(f(x)) .or. lambda <= 1e-10) then
                    exit
                else
                    lambda = lambda / 2
                end if
...
program main
    ...
    real(8), parameter :: step_size = 0.1d0, epsilon1 = 1e-12, epsilon2 = 1e-12
    type(result) :: root
    integer :: iter
    real(8) :: left, right
    character(50) :: file_name = "history_downhill.dat"
    character(20) :: seeker = "atiken"
    logical :: stat

    left = 1.0d0
    right = 4.0d0

    call newton_downhill(f, (left+right)/2, epsilon1, epsilon2, file_name, root, iter, stat)
    print *, iter
